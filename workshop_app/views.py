...
def ensure_profile(user):
    """Create a minimal profile when a user exists without one.

    This protects legacy/admin-created users that were added without
    going through the registration form.
    """
...
def activate_user(request, key=None):
    ...
    if key is None:
        ...
        if user.is_authenticated and not profile:
            logout(request)
            messages.add_message(request, messages.ERROR, "Your account profile is missing. Please register again.")
            return redirect(reverse("workshop_app:register"))

        if user.is_authenticated and not profile.is_email_verified and \
            ...
            return render(request, 'workshop_app/activation.html',
                          {'status': status})
        elif user.is_authenticated and not profile.is_email_verified:
            return render(request, 'workshop_app/activation.html')
        elif user.is_authenticated and profile.is_email_verified:
            status = "2"
            return render(request, 'workshop_app/activation.html',
                          {'status': status})
        else:
